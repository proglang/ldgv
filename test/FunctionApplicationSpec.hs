...
spec =
  describe "LDGV parser" $ do
    describe "type syntex" $ do
      it "parses Int->Int " $ do
        "type FUN : ~un = Int -> Int" `shouldParseDecl`
          DType "FUN" MMany Kun (TFun MMany "#!" TInt TInt)

      it "parses Int->Int->Int" $ do
        "type FUN : ~un = Int -> Int -> Int" `shouldParseDecl`
          DType "FUN" MMany Kun (TFun MMany "#!" TInt (TFun MMany "#!" TInt TInt))

      it "parses !Int.end" $ do
        "type T : ~lin = !Int.Unit" `shouldParseDecl`
          DType "T" MMany Klin (TSend "#!" TInt TUnit)

    describe "function application and let" $ do
