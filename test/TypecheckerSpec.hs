...
tcOptionsGradual :: Options
tcOptionsGradual = Options{ gradual = True }

spec :: Spec
spec = do
  describe "CCLDLC specific typechecking" $ do
    it "typechecking example function f1'" $ do
      let f = DFun "f" [] f1' Nothing
      typecheck tcOptionsCast [boolType, notFunc, f] `shouldBe` Right ()
    it "typechecking example function f2'" $ do
      let f = DFun "f" [] f2' Nothing
      typecheck tcOptionsCast [boolType, notFunc, f] `shouldBe` Right ()
    it "typechecking example function f" $ do
      let f' = DFun "f" [] f Nothing
      typecheck tcOptionsCast [boolType, notFunc, f'] `shouldBe` Right ()
    it "typechecking example function f3" $ do
      let f = DFun "f" [] f3 Nothing
      typecheck tcOptionsCast [boolType, notFunc, directionType, f] `shouldBe` Right ()
    it "typechecking example function f4'" $ do
      let f = DFun "f" [] f4' Nothing
      typecheck tcOptionsCast [boolType, notFunc, f] `shouldBe` Right ()
    it "typechecking example function f4" $ do
      let f = DFun "f" [] f4 Nothing
      typecheck tcOptionsCast [boolType, notFunc, f] `shouldBe` Right ()
    it "typechecking section 5.1 (2) function, expecting failure" $ do
